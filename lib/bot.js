      ...
      if (message.text.indexOf("@" + self.config.bot_name) == 0) {
        ...
        while (match = regexp.exec(message.text)) {
          ...
          // issue details in the response message:
              expandInfo = !!match[3];
          if(j.indexOf('-') === -1) {
              j = prjName + '-' + j.substring(prjName.length);
          }
            ...
            if (jiraApi && jiraApi != "NONE") {
              (function (def, jira) {
                ...
                jiraApi.findIssue(jira, function (error, issue) {
                  ...
                  if (issue && self.config.post == true) {
                    ...
                    if (self.config.showIssueDetails && expandInfo) {
                      ...
                      if (issue.fields.description) {
                        var descr = issue.fields.description;
                        descr = (descr.length >= self.config.maxDescriptionLength) ? descr.substr(0,self.config.maxDescriptionLength) + " . . ." : descr;
                        msg += '\n*Description*: _' + descr + '_';
                      }
                      if (issue.fields.priority && issue.fields.priority.name !== '') {
                        msg += '\n*Priority*: _' + issue.fields.priority.name + '_';
                      }
                      if (issue.fields.issuetype && issue.fields.issuetype.name !== '') {
                        msg += '\n*Type*: _' + issue.fields.issuetype.name + '_';
                        if (issue.fields.status && issue.fields.status.name !== '') {
                          msg += ' *Status*: _' + issue.fields.status.name + '_';
                        }
                      }
                  ...
                  var req = {"channel": message.channel, "color": "#ff0000","text": msg, "type": "issue"};
                  def.resolve(req);
                });
              })(q.defer(), j);
            } else {
